...
const RepoSelectionPage = () => {    
  ...
  const { user, apiClient } = useAuth(); // ← Utilisation du contexte d'authentification  
  ...
  useEffect(() => {    
    // Vérifier que l'utilisateur est authentifié et que apiClient est disponible  
    if (apiClient) {  
      loadRepos();    
    }  
  }, [apiClient]); // Dépendance sur apiClient  
